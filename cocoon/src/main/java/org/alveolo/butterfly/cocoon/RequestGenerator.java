...
public class RequestGenerator extends AbstractSAXGenerator {
	...
	private static final String ELEM_SESSION = "session";
	...
	public void execute() {
		...
		try {
			...
			startElement(ELEM_SESSION);
			writeAttributes(request.getSession());
			endElement(ELEM_SESSION);
	...
	private void writeAttributes(HttpSession session) throws JAXBException, SAXException {
		Enumeration<String> names = session.getAttributeNames();

		startElement(ELEM_ATTRIBUTES);
		while (names.hasMoreElements()) {
			String name = names.nextElement();
			writeAttribute(name, session.getAttribute(name));
		}
		endElement(ELEM_ATTRIBUTES);
	}
